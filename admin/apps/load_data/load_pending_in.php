...
if ($date != ''){
$query = "select * from sd_order_more WHERE activity = 1 AND date = '".$date."' ORDER BY id DESC";
}

elseif ($inID != ''){
$query = "select * from sd_order_more WHERE activity = 1 AND order_id = '".$inID."' ORDER BY id DESC";
}

elseif ($userID != ''){
$query = "select * from sd_order_more WHERE activity = 1 AND customerID = '".$userID."' ORDER BY id DESC";
}

else 
{
$query = "select * from sd_order_more WHERE activity = 1 ORDER BY id DESC";
}
		  ...
		  <thead>
             <tr class="info_member">
					...
					elseif ($inID != ''){
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, time from sd_order_more  
								WHERE activity = 1 AND order_id ='".$inID."'
                            ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
                        $stmt->bind_result($id, $order_id, $time);
						...
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, time from sd_order_more  
								WHERE activity = 1 AND customerID = '".$userID."'
                            ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
                        $stmt->bind_result($id, $order_id, $time);
                        ...
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, customer_name, mobile, g_total, date, l_time, activity 
								from sd_order_more WHERE activity = 1
                            ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
                    ...
                    while ($stmt->fetch()) {
						...
						$row_member = mysqli_fetch_assoc($member); 
                            ...
                            <tr>
								...
								if ($activity == 2){echo "<span style='color: #de0000;font-weight: bold;'>Completed</span>";}
								?>
                                </td>
                                <td> 
