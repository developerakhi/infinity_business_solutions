...
<div class="box-body table-responsive no-padding">
	<table class="table table-hover">
		...
		<tbody>
		 <?php 		
			global $mysqli;
			$stmt = $mysqli->prepare("SELECT menu_id, menu_name, shipping_charge, img1 from menu
				ORDER BY menu_id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage");
			$stmt->execute();    // Execute the prepared query.
			$stmt->bind_result($id, $menu_name, $shipping_charge, $img1);
			while ($stmt->fetch()) { ?>               
				<tr>
					<td><?php echo $id; ?></td>
					<td><img width="50px" src="../images/icon/<?php echo $img1; ?>" ></td>
