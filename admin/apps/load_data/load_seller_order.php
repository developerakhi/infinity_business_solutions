...
if ($date != ''){
$query = "select * from sd_order_more WHERE date = '".$date."' AND WHERE shop_id != 0 ORDER BY id DESC";
}

elseif ($inID != ''){
$query = "select * from sd_order_more WHERE order_id = '".$inID."' AND WHERE shop_id != 0 ORDER BY id DESC";
}

elseif ($userID != ''){
$query = "select * from sd_order_more WHERE customerID = '".$userID."' AND WHERE shop_id != 0 ORDER BY id DESC";
}

else 
{
$query = "select * from sd_order_more WHERE shop_id != 0 ORDER BY id DESC";
}
...
$query="select id, orderID from sd_order_list WHERE shop_id != 0 order by id DESC
             ...
             <tr class="info_member">
					...
					if ($date != ''){
                        ...
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, time from sd_order_more
								 WHERE date ='".$date."' AND WHERE shop_id != 0
					...
					elseif ($inID != ''){
                        ...
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, mathod from sd_order_more  
								WHERE order_id ='".$inID."' AND WHERE shop_id != 0
                            ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
						...
						global $mysqli;
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, mathod from sd_order_more  
								WHERE customerID = '".$userID."' AND WHERE shop_id != 0
                            ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
                        $stmt->bind_result($id, $order_id, $mathod);
						...
						global $mysqli;		
                        if ($stmt = $mysqli->prepare("SELECT id, order_id, customer_name, mobile, g_total, mathod, trxid, acc_no, date, activity from sd_order_more 
                            WHERE shop_id != 0 ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage")){
                        $stmt->execute();    // Execute the prepared query.
                        // get variables from result.
