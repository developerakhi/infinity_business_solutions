...
$query="select link from banner order by id DESC
...
?>
 <div class="box-body table-responsive no-padding">
	...
	<table class="table table-hover">
	 ...
	 <?php 		
		global $mysqli;
		$stmt = $mysqli->prepare("SELECT id, link, img1 from banner
    	    ORDER BY id DESC limit ".mysqli_real_escape_string($mysqli,$start).",$recordsPerPage");
        $stmt->execute();    // Execute the prepared query.
        // get variables from result.
        $stmt->bind_result($id, $link, $img1);
		
				while ($stmt->fetch()) { ?>               
                            <tr>
                                <td><?php echo $id; ?></td>
                                <td><img style="width: 50px;" src="../images/banner/<?php if ($img1 == NULL){echo 'photo.png';}else{echo ($img1);} ?>" /></td>
								<td><?php echo $link; ?></td>
                                <td style="text-align: center;">
								<a href="update_banner/<?php echo $id; ?>" class="btn btn-success btn-raised btn-xs" data-toggle="tooltip" data-placement="top" title="Edit Information" ><i class="fa fa-pencil"></i><div class="ripple-container"></div></a>
                                
                              <a href="apps/bin_cat/delete_banner.php?actionID=all_banner&catID=<?php echo $id; ?>" class="btn btn-danger btn-raised btn-xs" data-toggle="tooltip" data-placement="top" title="Delete This Row" onClick="return confirm('Are you sure to Delete ?')"><i class="fa fa-close"></i><div class="ripple-container"></div></a>
							</td>
                            </tr>
   						 	 <?php }
							   $stmt->close();
